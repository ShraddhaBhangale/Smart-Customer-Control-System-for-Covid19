...
class MotionSimulator:
    ...
    def leave_the_store(self):
        ...
        res = self.send_post_request(send_dict)
        #self.send_post_request_toqueueweb(send_dict) 
        return self.get_httpreponse_current_customers(res)
    
    def send_post_request(self,input_dict): # added for illustration purpose
        return requests.post(url = self.API_LOCATION, data = input_dict)

    def send_post_request_toqueueweb(self,input_dict):# added for illustration purpose
